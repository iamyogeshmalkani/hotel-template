...
export default function RoomsPage() {
    ...
    return (
        <div className='py-20 flex flex-col justify-center items-center gap-10 font-Lora '>
            ...
            <div className='flex flex-row flex-wrap gap-5 w-[90%]'>
                {
                    Object.keys(HOTEL_ROOM_TYPES_AND_INFO).map((type, ind) => {
                        return (
                            <div key={ind} className=''> <ProductCard clickHandler={() => clickHandler(type)} heading={type} description={HOTEL_ROOM_TYPES_AND_INFO[type].description} imageLocation={"images/room-types/" + HOTEL_ROOM_TYPES_AND_INFO[type].photos[0] + ".jpg"} />
                            </div>)
                    })
                }
            </div>
